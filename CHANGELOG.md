...
- **`MetaspaceLeakIT` A/B 对比审计完善（2026-09-09）**：
  ...
  - **泄漏判定执行顺序调整：诊断先行→断言殿后（2026-09-09）**：原主断言 `assertThat(classLoaderCount).isZero()` 放在 WARN 提醒和 heap dump 之前，断言失败时后续 dump 不执行，无法定位根因。调整为：WARN 提醒 → heap dump（条件 `retained > 0 || CL > 0`）→ 硬断言殿后，确保断言失败前 dump 已生成
  - **`printClassCountDiff` 判定文字结合 CL 计数（2026-09-09）**：原 `classes => 类元数据未卸载证候` 判定文字仅依据 retained classes，CL=0 时仍打印"成立（Metaspace 泄漏）"误导。修复：`printClassCountDiff` 新增 `classLoaderCount` 参数，判定逻辑改为：CL<0 → "采样失败"，retained<=0 → "不成立"，retained>0 && CL==0 → "有残留但 CL=0（非泄漏）"，retained>0 && CL>0 → "成立（CL 拘留+类未卸载=Metaspace 泄漏）"
  - **`isJobRecentlyFinished` finished-job-state 延迟窗口（2026-09-09）**：CI 实测（run #34361098891）Agent 组 45 job 中 ~40 个 "Falling back to container log"——`isJobInRunningJobIMap` 全部返回 false（job FINISHED 后引擎立即从 `running-job` 移除并转入 `finished-job-state`，检查 `running-job` IMap 永远不命中，延迟释放从未生效）。修复：新增 `isJobRecentlyFinished` 方法检查 `finished-job-state` IMap + 5 秒时间窗口延迟（`FINISHED_JOB_FIRST_SEEN` ConcurrentMap 追踪首次发现时间，`FINISHED_JOB_DEFERRAL_MS = 5000L`），`forceEvictJobClassLoaders` 改为 `isJobInRunningJobIMap(jobId) || isJobRecentlyFinished(jobId)`——作业在 finished-job-state 出现后 5 秒内延迟释放 ClassLoader，给 REST API 查询留出时间，超时后放行避免永驻泄漏
  - **suspicious result 条件再调整（2026-09-09）**：CI 实测（run #34361098891）Agent 和 Native 两组均触发 suspicious result——`otherDead > 100` 太激进，Native-Control otherDead=1245 也触发（大量 dead DelegatingClassLoader 在两组都属正常）。修复：移除 `otherDead > 100` 条件，suspicious 条件改为 `parsedLines == 0 || bootstrapClasses == 0 || otherClasses > 100000L`
  - **"Leak evidence" WARN 降为 INFO（2026-09-09）**：CI 实测 CL=0（非泄漏）时仍打 WARN "Leak evidence" 误导诊断。修复：改为 INFO 级别——CL=0 时 retained classes 属 bootstrap/AppCL 正常驻留，不应 WARN
  - **`isJobInRunningJobIMap`/`isJobRecentlyFinished` IMap key 类型双查（2026-09-09）**：CI 实测（run #34365541571）`isJobInRunningJobIMap` 57 次全部返回 false、"Falling back to container log" 80 次、`isJobRecentlyFinished` 0 次日志，但 REST API 查 `running-job` 返回 `{"jobId":"xxx"}` 有条目——根因：两个方法只用 `Long` key 调 `containsKey`，但 Hazelcast IMap 的实际 key 可能是 `String` 类型（`cleanHazelcastJobState` 的 remove 已同时尝试 Long+String 两种 key，但 `containsKey` 漏了 String）。修复：两个方法的 `containsKey` 同时尝试 `Long` 和 `String` 两种 key；`isJobRecentlyFinished` 补全 job 不在 IMap 时的 INFO 诊断日志（此前该分支静默 return false 致 0 次日志无法判断方法是否被调用）
  - **Hazelcast IMap 名称全面修正（2026-09-09）**：读 SeaTunnel 源码（`Constant.java`）发现 IMap 实际名称为 `engine_runningJobInfo`/`engine_runningJobState`/`engine_finishedJobState` 等（key 类型 `Long`），此前代码用的 `running-job`/`finished-job-state` 是 REST URL 路径段而非 IMap 名称。进一步读 `JobInfo`/`JobState`/`JobDAGInfo`/`JobMetrics`/`VertexInfo` 源码确认所有 `engine_` 前缀 IMap 的 value 均为纯数据类型（`Serializable`），Hazelcast 以序列化 `Data` 存储，不持有 ClassLoader GC Root——清理它们对 ClassLoader 回收无帮助，只破坏引擎状态管理和 REST API 查询。真正持有 ClassLoader 的是 Java 对象 Map（`classLoaderCache`/`runningJobMasterMap`/JobMaster 内部字段），已由 `forceEvictJobClassLoaders`/`cleanJobMaster` 处理。修复：(1) `isJobInRunningJobIMap` 改查 `engine_runningJobInfo`+`engine_runningJobState`；(2) `isJobRecentlyFinished` 改查 `engine_finishedJobState`；(3) `cleanHazelcastJobState` 排除所有 `engine_` 前缀 IMap（此前排除条件 `!name.contains("finished-job-state")`/`!name.equals("running-job")` 永远 true，仍在误删所有 engine_ IMap 致 REST API 返回 `{"jobId":"xxx"}` 无 jobStatus）
  - **根因修复：删除 `cleanHazelcastJobState` + `isJobInRunningJobIMap` + `isJobRecentlyFinished`（2026-09-09）**：上述 IMap 名称修正仍是补丁——读 SeaTunnel 源码（`JobInfoService.getJobInfoJson`）确认 REST API 返回 `{"jobId":"xxx"}` 的真正原因是 `cleanHazelcastJobState` 误删 IMap 条目（不是 ClassLoader 释放致序列化失败，Hazelcast IMap 存序列化 `Data` 不依赖 connector ClassLoader）。所有 `engine_` IMap value 均为纯数据（`Serializable`），不持有 ClassLoader GC Root，`cleanHazelcastJobState` 清理它们对 ClassLoader 回收无帮助只破坏 REST API。`isJobInRunningJobIMap`/`isJobRecentlyFinished` 基于错误假设（"ClassLoader 释放致序列化失败"）添加，CI 57 次全 false 从未生效，竞态防护由 `activeJobIds` 过滤（查 `runningJobMasterMap` Java 对象 Map）负责。根因修复：删除 `cleanHazelcastJobState` 方法及其在 `cleanJobMaster` 中的调用、删除 `isJobInRunningJobIMap`/`isJobRecentlyFinished` 方法及 `FINISHED_JOB_FIRST_SEEN`/`FINISHED_JOB_DEFERRAL_MS` 字段、`forceEvictJobClassLoaders` 移除延迟检查直接释放 ClassLoader、清理 5 个未使用 import（`DistributedObject`/`IMap`/`Hazelcast`/`HazelcastInstance`/`Collection`）
